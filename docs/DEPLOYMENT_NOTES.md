...
The App Service ZIP package is generated by the deployment script with portable `/` entry names. Do not replace this with Windows `Compress-Archive`; Linux Kudu rsync can fail on Windows-style `\` entry names. Local development certificates under `.certificates` are excluded from publish and deploy packages.
