...
At startup, the API loads environment variables, initializes SQLite schema, registers services, starts the hosted crawler, and exposes minimal API endpoints.

### 4.2 Crawling and Scraping

The crawler discovers Oyak Dijital URLs from robots, sitemap, direct root URLs, HTML links, and rendered page links. Playwright renders pages like a browser, waits for stabilization, captures `document.body.innerText`, and extracts links.
...
Cleaned pages are stored in SQLite with URL, title, content, content hash, first seen time, last seen time, and last crawl time. Stale pages are deleted in incremental flows, and full refresh uses backup/restore to protect current data.
...
Ready questions are generated by the active AI provider from the current knowledge payload. The table is replaced as a single current set whenever source fingerprint changes or a force refresh runs. The frontend receives a small rotating subset.
...
- Header with new chat, Oyak Dijital brand link, visitor menu, and settings.
...
- Q&A board with accessible `Siz` and `Oyako` headings.
...
- Settings dialog with AI provider/model selection and save progress.
