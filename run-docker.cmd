...
docker run --rm --init -it --name "%CONTAINER%" --label "%LABEL%" --label "com.oyako.role=fullstack-docker-dev" -p 5000:5000 -p 3000:3000 --env-file "%ROOT%azure-cloud.env" --env-file "%ROOT%ollama-cloud.env" -e OYAKO_DOCKER=1 -e ASPNETCORE_ENVIRONMENT=Production -e Ai__DefaultProvider=ollama-cloud -e Ai__DisabledProviders__0=ollama-local --ipc=host "%IMAGE%"
...
:purge_oyako
powershell -NoProfile -ExecutionPolicy Bypass -Command "$label='com.oyako.app=oyako'; $tag='oyako:docker-dev'; docker ps -aq --filter ('label=' + $label) 2>$null | ForEach-Object { if($_){ docker rm -f $_ 2>$null | Out-Null } }; docker images -q --filter ('label=' + $label) 2>$null | Sort-Object -Unique | ForEach-Object { if($_){ docker image rm -f $_ 2>$null | Out-Null } }; docker image rm -f $tag 2>$null | Out-Null"
exit /b 0
